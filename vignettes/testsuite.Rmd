...
```{r,error=TRUE}
ph = phonto::nhanesTables('EXAM', 2008)
...
In the above test case, the dimension of the `phonto::nhanesA::nhanesTables()` miss matched `nhanesA::nhanesTables()`.
Further, we found that `nhanesA` has an extra row because the database intentionally removed some of the tables.

```{r,error=TRUE}
setdiff(nhsA$Data.File.Name,ph$Data.File.Name)
```

It could go the other way, `phonto/DB` shows more tables than `nhanesA.` For example, `phonto` shows three more tables than `nhanesA.`  Since `nhanesA` scrape data from the CDC website,  it is possible no longer exists/downloadable on the CDC website. 

```{r,error=TRUE}
ph_lab = phonto::nhanesTables('LAB', 2008)
nhsA_lab = nhanesA::nhanesTables('LAB', 2008)
diff_tables = setdiff(ph_lab$Data.File.Name,nhsA_lab$Data.File.Name)
```
