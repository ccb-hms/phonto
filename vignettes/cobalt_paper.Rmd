...
```
 FIXME: Laha - can you please spend some time and try to replicate table 1 - I think for the paper it is probably enough just to do column 1, 
 
```{r replicateCol1Table1}
 
 table(demo_sub$PL)
 table(demo_sub$DMDEDUC2)
 table(demo_sub$smokingVar)
 table(demo_sub$RIDRETH1)
 table(demo_sub$RIAGENDR)
 
 mean(demo_sub$BMXBMI)
 sd(demo_sub$BMXBMI)
 mean(demo_sub$RIDAGEYR)
 sd(demo_sub$RIDAGEYR)
```

### FIXME: below here we will need to switch the analysis to just use the data.frame demo_sub - easiest is to replace base_df with it.
## Glucose
