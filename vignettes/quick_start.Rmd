...
This function lets users search for tables in the database using the table name.  
...
#### 2.3 Searching for Variables
 ...
 The `nhanesSearch` function searches for text strings in the variable names and then returns the name of the table that contains
...
The `nhanesSearch` function also has a way to exclude certain matches. In the search above any variable name that has the word "during" in it will also match and we probably don't want those. So in the output below we exclude those terms.
```{r}
...
```{r}
s1 = nhanesSearch("LDL", nchar=256)
##from this one can obtain Variable names
s1[1,c(2:3)]
##FIXME: how did you get to LBDLDL from the above? this doc should explain
##everything....
